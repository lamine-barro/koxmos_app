...
export async function streamTextTutor(input: { firstName: string; country: string; tutorKey?: TutorKey; skill?: string; skillLevel?: SkillLevel; learningSessionId?: string; clientContext?: string; message: string }, onEvent: (event: TextTutorStreamEvent) => void, signal?: AbortSignal): Promise<void> {
  ...
  const response = await transportFetch(url, { method: 'POST', signal, headers: { Accept: 'text/event-stream', 'Content-Type': 'application/json', 'X-Koxmos-Device-Id': deviceId }, body: JSON.stringify(input) });
  if (!response.ok) { const data = await response.json().catch(() => ({})); throw new AgentRequestError(data.error || 'Le service est indisponible.', response.status); }
