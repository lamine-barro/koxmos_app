...
function VoiceLayout({ skill, tutor, messages, textMode, draft, setDraft, working, chat, leave, switchAgent }: { skill?: Skill; tutor: Tutor; messages: Message[]; textMode: boolean; draft: string; setDraft: (value: string) => void; working: boolean; chat: () => void; leave: () => void; switchAgent: () => void }) {
  return <SafeAreaView style={voice.screen} edges={['top', 'bottom']}><StatusBar style="light" /><View style={voice.body}><View style={voice.tutor}><View style={voice.avatarWrap}><View style={voice.avatarRing} /><Image source={tutor.avatar} style={voice.avatar} /></View><Text style={voice.kicker}>VOTRE TUTEUR</Text><Text style={voice.name}>{tutor.name}</Text><Text style={voice.question}>{skill?.name || 'Votre compétence'}</Text></View><View style={voice.conversation}><Text style={voice.hint}>Un même canvas rassemble la transcription vocale et les échanges texte.</Text><ConversationCanvas messages={messages} tutor={tutor} dark /></View></View>{textMode && <View style={voice.chatForm}><TextInput value={draft} onChangeText={setDraft} placeholder={`Écrire à ${tutor.name}`} placeholderTextColor="#737373" multiline style={voice.chatInput} /><Pressable accessibilityRole="button" accessibilityLabel="Envoyer le message" disabled={!draft.trim() || working} onPress={chat} style={[voice.sendButton, (!draft.trim() || working) && voice.sendButtonDisabled]}><ArrowRight size={21} color="#000000" /></Pressable></View>}<View style={voice.footer}><Pressable accessibilityRole="button" accessibilityLabel="Fermer la conversation" onPress={leave} style={voice.closeButton}><X size={22} color="#FFFFFF" /></Pressable><Pressable accessibilityRole="button" accessibilityLabel={textMode ? 'Passer au tuteur vocal' : 'Passer au tuteur texte'} accessibilityState={{ selected: textMode }} onPress={switchAgent} style={[voice.muteButton, textMode && voice.textButtonActive]}>{textMode ? <MessageCircle size={23} color="#FFFFFF" /> : <LucideMic size={23} color="#FFFFFF" />}</Pressable></View></SafeAreaView>;
}
...
const s = StyleSheet.create({
  ...
  metricRow: { flexDirection: 'row', gap: 12 }, metricCard: { flex: 1, minHeight: 112, borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 16, padding: 14, justifyContent: 'space-between' }, metricHeader: { flexDirection: 'row', alignItems: 'center', gap: 8 }, metricIcon: { width: 32, height: 32, borderRadius: 16, alignItems: 'center', justifyContent: 'center' }, flameCard: { backgroundColor: '#FFFBEB' }, flameIcon: { backgroundColor: '#FEF3C7' }, walletCard: { backgroundColor: '#F8FAFC' }, walletIcon: { backgroundColor: '#EFF6FF' }, metricLabel: { flex: 1, color: '#6B7280', fontSize: 12, lineHeight: 16, fontWeight: '700' }, metricValue: { color: '#111827', fontSize: 25, lineHeight: 30, fontWeight: '800', letterSpacing: -.5 }, flameCount: { color: '#111827', fontSize: 24, fontWeight: '800' }, flameLabel: { color: '#6B7280', fontSize: 12 }, cardAction: { flexDirection: 'row', alignItems: 'center', gap: 4 }, cardActionText: { color: '#4B5563', fontSize: 13 }, flameRule: { color: '#6B7280', fontSize: 12, marginTop: -8 }, modeGroup: { flexDirection: 'row', padding: 4, gap: 4, borderRadius: 14, backgroundColor: '#F3F4F6' }, modeButton: { flex: 1, minHeight: 44, borderRadius: 10, justifyContent: 'center', alignItems: 'center' }, modeButtonActive: { backgroundColor: '#111827' }, modeText: { color: '#4B5563', fontWeight: '800' }, modeTextActive: { color: '#fff' },
