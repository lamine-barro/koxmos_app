...
export default function HomeScreen() {
  ...
  async function createProfile() {
    ...
    setWorking(true);
    try {
      const created = await saveProfile(firstName, country);
      setProfile(created);
      setFlame(await loadFlame().catch(() => 0));
      setWallet(await loadWallet().catch(() => null));
      setPage('home');
  ...
  function renderHome() {
    ...
    return <HomeContent
      ...
      startConversation={() => {
        if (!tutor || !selected) return;
        Alert.alert(
          'Démarrer la conversation ?',
          `${tutor.name} ouvrira un échange audio sur « ${selected.name} » et vous proposera de travailler ou de commencer une évaluation.`,
          [
            { text: 'Annuler', style: 'cancel' },
            { text: 'Oui, démarrer', onPress: () => { debugLog('conversation.voice.confirmed_from_home', { skill: selected.name, tutor: tutor.key }); setMessages([]); setVoiceAutoStart(true); setPage('voice'); } },
          ],
        );
      }}
  ...
  if (page === 'profile-details') addLayer('profile-details', <ProfileDetailsScreen profile={profile} save={async (name, nextCountry, saveConversationHistory) => { const updated = await saveProfile(name, nextCountry, { saveConversationHistory }); if (!saveConversationHistory) await clearConversationHistory(); setProfile(updated); notify(tr('profileSaved')); }} deletePassport={async () => { await deleteRemoteAccount().catch(() => undefined); await deleteLocalPassport(); setProfile(null); setSkills([]); setSelected(undefined); setTutor(undefined); setWallet(null); setFlame(0); setMessages([]); setLearningSession(undefined); setTextConversationCredits(0); setFirstName(''); setCountry('CI'); setPage('home'); }} back={() => setPage('profile')} />);
...
function VoiceLayout({ tutor, messages, agentAudioLevel, textMode, draft, setDraft, working, chat, leave, switchAgent }: { skill?: Skill; tutor: Tutor; messages: Message[]; agentAudioLevel: number; textMode: boolean; draft: string; setDraft: (value: string) => void; working: boolean; chat: () => void; leave: () => void; switchAgent: () => void }) {
  const { tr } = useCopy();
