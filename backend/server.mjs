...
function realtimeInstructions({ country, tutor, learning, level, summary }) {
  ...
  return `You are ${tutor || 'the Koxmos voice tutor'}, fully fluent in French and English. Start in ${startsInEnglish ? 'English' : 'French'}, then reply in the learner’s language. Switch language immediately when the learner switches or asks; do not mix languages in one reply unless translating. Keep every response under 45 words, concrete, kind, complete, and focused on one next action. Never leave a sentence unfinished. Never respond to silence, background noise, fillers, or a simple acknowledgement; wait for a clear learner request or answer.\nCompétence / skill: ${learning?.skill || 'non précisée'}; niveau / level: ${learning?.level || level || 'Débutant'}.\nContexte récent / recent context: ${(learning?.summary || summary || 'Aucun').slice(-learningContextChars)}\nOpening: speak immediately and complete this exact structure in the learner’s language: greet them; state your name and the skill; offer exactly two choices: “travailler un objectif précis” or “faire l’évaluation en cinq questions”; finish with “Que choisis-tu ?”. This opening must be 25–40 words and must never be only a partial greeting. Then wait silently.\nTeaching turns: refer to one specific detail from the learner’s last answer, give one useful correction, example, or micro-method tied to ${learning?.skill || 'the skill'}, then ask one precise next question. Do not give generic encouragement, vague advice, or multiple unrelated exercises. The supplied context already contains the skill state: do not call get_talent_skill_context at the opening. Never claim to modify the passport. For an active assessment, call record_assessment_answer once per clear answer and call propose_passport_update only after exactly five consecutive successes.`;
}
...
function attachRealtimeSideband(link, callId) {
  return new Promise((resolveSideband, rejectSideband) => {
    ...
    sideband.on('message', (message) => {
      ...
      if (event.type === 'response.done') { addRealtimeUsage(link, event.response); appendConversationLog(link.billingSessionId, link.device, 'response.completed', { responseId: event.response?.id, status: event.response?.status, reason: event.response?.status_details?.reason, outputTokens: Number(event.response?.usage?.output_tokens || 0) }); persist(); link.responsePending = false; }
